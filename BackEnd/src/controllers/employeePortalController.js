...
const getMyLivePayrollDailyDetails = asyncHandler(async (req, res) => {
  ...
  // Verify this employee is part of this payroll run and get client_id
  const [recordCheck] = await db.execute(`
    SELECT pr.id, prn.client_id
    FROM payroll_records pr
    JOIN payroll_runs prn ON pr.run_id = prn.id
  ...
  try {
    const dailyDetails = await payrollService.getEmployeeDailyWorkDetails(runId, employeeId, clientId);
