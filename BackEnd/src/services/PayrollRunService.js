...
class PayrollRunService {
    ...
    // =============================================
    // LIVE PAYROLL CALCULATION METHODS
    // =============================================

    /**
     * Calculate live/real-time payroll for an employee up to current date
     * @param {string} employeeId - Employee ID
     * @param {string} clientId - Client ID
     ...
     */
    async calculateLivePayroll(employeeId, clientId, calculateUpTo = null, projectFullMonth = false) {
        ...
        try {
            ...
            // Get employee payroll data (reuse existing method)
            const employeeData = await this.getEmployeePayrollData(employeeId, clientId);
            if (!employeeData) {
                throw new Error('Employee not found or not eligible for payroll');
            }

            // Create a temporary payroll record structure for calculations
            const tempRecord = {
                ...
                calculation_method: 'live',
                period_start: startOfMonth.format('YYYY-MM-DD'),
                period_end: endDate.format('YYYY-MM-DD')
            };

            // Get attendance data for the period (modify existing method for date range)
            const attendanceRecords = await this.getAttendanceForPeriod(
                employeeId,
                clientId,
            ...
            // Calculate attendance summary (reuse existing method)
            const workingHours = await this.getEmployeeDailyHours(employeeId, 8);
            ...
            const grossComponents = await this.calculateGrossComponents(tempRecord, employeeData);

            // Calculate deductions (reuse existing method)
            const deductions = await this.calculateDeductions(grossComponents.total, 'advanced', employeeData);

            // Calculate net salary
            const netSalary = grossComponents.total - deductions.total;

            // Project full month if requested and not end of month
            let projectedData = null;
            if (projectFullMonth && endDate.date() < endOfMonth.date()) {
                projectedData = await this.projectFullMonthSalary(
                    employeeData,
                    attendanceSummary,
                    startOfMonth,
                    endOfMonth,
            ...
            return {
                ...
                data: {
                    ...
                    period: {
                        ...
                        completion_percentage: Math.round((daysCalculated / totalDaysInMonth) * 100)
                    },
                    attendance: {
                        days_worked: attendanceSummary.total_working_days || 0,
                        total_hours: attendanceSummary.total_hours || 0,
                        overtime_hours: attendanceSummary.overtime_hours || 0,
                        leave_days: attendanceSummary.approved_leave_days || 0,
                        absent_days: attendanceSummary.absent_days || 0
                    },
                    earnings: {
                        base_salary_prorated: proratedBaseSalary,
                        allowances: grossComponents.allowances || 0,
                        overtime: grossComponents.overtime || 0,
                        bonuses: grossComponents.bonuses || 0,
                        total: grossComponents.total
                    },
                    deductions: {
                        tax: deductions.tax || 0,
                        provident_fund: deductions.provident_fund || 0,
                    ...
                    summary: {
                        gross_salary: grossComponents.total,
                        total_deductions: deductions.total,
                        net_salary: Math.round(netSalary * 100) / 100,
                        proration_factor: Math.round(prorationFactor * 100) / 100
                    },
     ...
     */
    async getAttendanceForPeriod(employeeId, clientId, startDate, endDate) {
        const db = getDB();
    ...
    /**
     * Project full month salary based on current performance
     */
    async projectFullMonthSalary(employeeData, attendanceSummary, startOfMonth, endOfMonth, daysCalculated) {
        const moment = require('moment');

        try {
            // Calculate average performance metrics
            const avgWorkingDaysPerCalculatedDay = (attendanceSummary.total_working_days || 0) / daysCalculated;
            const avgHoursPerDay = (attendanceSummary.total_hours || 0) / Math.max(1, attendanceSummary.total_working_days || 1);
            const avgOvertimePerDay = (attendanceSummary.overtime_hours || 0) / daysCalculated;

            // Project for remaining days in month
            const totalDaysInMonth = endOfMonth.date();
            const remainingDays = totalDaysInMonth - daysCalculated;

            // Calculate projected totals
            const projectedTotalWorkingDays = Math.round(
                (attendanceSummary.total_working_days || 0) + (remainingDays * avgWorkingDaysPerCalculatedDay)
            );
            const projectedTotalHours = Math.round(
                ((attendanceSummary.total_hours || 0) + (remainingDays * avgWorkingDaysPerCalculatedDay * avgHoursPerDay)) * 100
            ) / 100;
            const projectedOvertimeHours = Math.round(
                ((attendanceSummary.overtime_hours || 0) + (remainingDays * avgOvertimePerDay)) * 100
            ) / 100;

            // Create projected record for calculations
            ...
            // Calculate projected gross components
            const projectedGross = await this.calculateGrossComponents(projectedRecord, employeeData);

            // Calculate projected deductions
            const projectedDeductions = await this.calculateDeductions(projectedGross.total, 'advanced', employeeData);

            // Calculate projected net salary
            const projectedNetSalary = projectedGross.total - projectedDeductions.total;

            // Calculate confidence level (higher confidence with more days calculated)
            const confidenceLevel = Math.min(95, Math.max(50, (daysCalculated / totalDaysInMonth) * 100));

            return {
                projected_working_days: projectedTotalWorkingDays,
                projected_total_hours: projectedTotalHours,
                projected_overtime_hours: projectedOvertimeHours,
                projected_gross_salary: Math.round(projectedGross.total * 100) / 100,
                projected_total_deductions: Math.round(projectedDeductions.total * 100) / 100,
                projected_net_salary: Math.round(projectedNetSalary * 100) / 100,
                confidence_level: Math.round(confidenceLevel),
                projection_note: `Based on ${daysCalculated} days of performance data`
            };

        } catch (error) {
            console.error('❌ Error projecting full month salary:', error);
            return null;
        }
    }

    /**
     * Calculate live payroll for date range (for resignations, joinings etc)
     */
    async calculateDateRangePayroll(employeeId, clientId, startDate, endDate) {
        const moment = require('moment');
        ...
        try {
            ...
            return {
                ...
                data: {
                    ...
                    summary: {
                        gross_salary: Math.round(grossComponents.total * 100) / 100,
                        total_deductions: Math.round(deductions.total * 100) / 100,
                        net_salary: Math.round(netSalary * 100) / 100
                    },
                    calculated_at: moment().toISOString()
                }
