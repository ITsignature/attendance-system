...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='Admin users table with support for email or mobile login';
...
CREATE TABLE `attendance` (
  ...
  `break_start_time` time DEFAULT NULL COMMENT 'Actual break start time during shift',
  `break_end_time` time DEFAULT NULL COMMENT 'Actual break end time during shift',
  ...
  `payable_duration` int(11) DEFAULT NULL COMMENT 'Payable duration in seconds (for precision)',
  `pre_shift_overtime_seconds` int(11) DEFAULT 0 COMMENT 'Actual seconds worked before scheduled start time as overtime',
  `post_shift_overtime_seconds` int(11) DEFAULT 0 COMMENT 'Actual seconds worked after scheduled end time as overtime'
...
CREATE TABLE `employees` (
  ...
  `fingerprint_id` int(11) DEFAULT NULL COMMENT 'Maps to fingerprint scanner ID. Must match the ID stored in AS608 fingerprint device.',
  ...
  `break_start_time` time DEFAULT NULL COMMENT 'Employee break start time',
  `break_end_time` time DEFAULT NULL COMMENT 'Employee break end time',
  ...
  `payroll_cycle_override` enum('default','custom') DEFAULT 'default' COMMENT 'Whether employee uses default or custom payroll cycle',
  `payroll_cycle_day` int(11) DEFAULT NULL COMMENT 'Day of month when payroll cycle starts (1-31). NULL = use client default from payroll_periods',
  `payroll_cycle_effective_from` date DEFAULT NULL COMMENT 'Date when custom cycle becomes effective (for smooth transitions)',
  `overtime_enabled` tinyint(1) DEFAULT 1 COMMENT 'Enable/disable overtime salary calculation for this employee',
  `pre_shift_overtime_enabled` tinyint(1) DEFAULT 0 COMMENT 'Enable overtime calculation for hours worked before shift start time',
...
CREATE TABLE `fingerprint_devices` (
  ...
  `uptime_minutes` int(11) DEFAULT NULL,
  `current_mode` varchar(20) DEFAULT 'attendance',
...
CREATE TABLE `leave_requests` (
  ...
  `dayofweek` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin DEFAULT NULL COMMENT 'Day-of-week breakdown: {"1": 0, "2": 1, ...} where 1=Sunday, 2=Monday, ..., 7=Saturday' CHECK (json_valid(`dayofweek`)),
  `payable_leave_hours_weekday` decimal(10,2) DEFAULT 0.00 COMMENT 'Payable hours for weekdays (Mon-Fri) covered by this leave',
  `payable_leave_hours_saturday` decimal(10,2) DEFAULT 0.00 COMMENT 'Payable hours for Saturdays covered by this leave',
  `payable_leave_hours_sunday` decimal(10,2) DEFAULT 0.00 COMMENT 'Payable hours for Sundays covered by this leave'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `leave_types` (
  ...
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  `tracking_period` enum('Monthly','Yearly') DEFAULT 'Yearly' COMMENT 'whether to track leaves are checked which period',
  `max_days_per_month` int(11) DEFAULT 0 COMMENT 'to track max days per month an employee can take monthly tracking leave type',
  `is_trainee_only` tinyint(1) DEFAULT 0,
  `half_day_carryover_enabled` tinyint(1) DEFAULT 0
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `payroll_records` (
  `id` varchar(36) NOT NULL DEFAULT uuid(),
  `run_id` varchar(36) NOT NULL,
  `employee_period_start_date` date DEFAULT NULL COMMENT 'Actual period start date used for this employee (may differ from run period for custom cycles)',
  `employee_period_end_date` date DEFAULT NULL COMMENT 'Actual period end date used for this employee (may differ from run period for custom cycles)',
  `uses_custom_cycle` tinyint(1) DEFAULT 0 COMMENT 'Flag indicating if this employee used custom cycle (1) or default (0) for this run',
  `employee_id` varchar(36) NOT NULL,
  `employee_code` varchar(50) NOT NULL,
...
CREATE TABLE `payroll_runs` (
  ...
  `run_status` enum('draft','calculating','calculated','partially_processed','processing','completed','cancelled') DEFAULT NULL,
...
ALTER TABLE `employees`
  ...
  ADD KEY `idx_employees_fingerprint_id` (`fingerprint_id`),
  ADD KEY `idx_employees_overtime_enabled` (`overtime_enabled`),
  ADD KEY `idx_employees_payroll_cycle` (`payroll_cycle_override`,`payroll_cycle_day`);
...
ALTER TABLE `fingerprint_devices`
  ...
  ADD KEY `idx_fd_client` (`client_id`),
  ADD KEY `idx_fd_device_id` (`device_id`),
  ADD KEY `idx_fd_online` (`is_online`);
...
ALTER TABLE `leave_requests`
  ...
  ADD KEY `idx_leave_requests_payable_hours` (`employee_id`,`status`,`payable_leave_hours_weekday`,`payable_leave_hours_saturday`,`payable_leave_hours_sunday`);
...
ALTER TABLE `payroll_records`
  ...
  ADD KEY `idx_payroll_records_custom_cycle` (`uses_custom_cycle`,`employee_period_start_date`,`employee_period_end_date`);
...
--
-- Constraints for table `fingerprint_devices`
--
ALTER TABLE `fingerprint_devices`
  ADD CONSTRAINT `fk_fd_client` FOREIGN KEY (`client_id`) REFERENCES `clients` (`id`) ON DELETE CASCADE;
