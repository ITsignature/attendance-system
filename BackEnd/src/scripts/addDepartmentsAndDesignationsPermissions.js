const mysql = require('mysql2/promise');
const { v4: uuidv4 } = require('uuid');
require('dotenv').config({ path: '.env' });

/**
 * Migration Script: Add Departments and Designations Permissions
 *
 * This script adds permissions for:
 * - Departments module (view, create, edit, delete)
 * - Designations module (view, create, edit, delete)
 */

const addDepartmentsAndDesignationsPermissions = async () => {
  const connection = await mysql.createConnection({
    host: process.env.DB_HOST,
    port: parseInt(process.env.DB_PORT),
    user: process.env.DB_USER,
    password: process.env.DB_PASSWORD,
    database: process.env.DB_NAME
  });

  try {
    console.log('🔄 Starting departments and designations permissions migration...');
    console.log(`📡 Connected to database: ${process.env.DB_NAME} at ${process.env.DB_HOST}`);

    // Define the new permissions to add
    const newPermissions = [
      // Departments permissions
      {
        module: 'departments',
        action: 'view',
        name: 'View Departments',
        description: 'View departments list and details'
      },
      {
        module: 'departments',
        action: 'create',
        name: 'Create Departments',
        description: 'Create new departments'
      },
      {
        module: 'departments',
        action: 'edit',
        name: 'Edit Departments',
        description: 'Modify existing departments'
      },
      {
        module: 'departments',
        action: 'delete',
        name: 'Delete Departments',
        description: 'Remove departments from system'
      },
      // Designations permissions
      {
        module: 'designations',
        action: 'view',
        name: 'View Designations',
        description: 'View designations list and details'
      },
      {
        module: 'designations',
        action: 'create',
        name: 'Create Designations',
        description: 'Create new designations'
      },
      {
        module: 'designations',
        action: 'edit',
        name: 'Edit Designations',
        description: 'Modify existing designations'
      },
      {
        module: 'designations',
        action: 'delete',
        name: 'Delete Designations',
        description: 'Remove designations from system'
      }
    ];

    // Insert new permissions (IGNORE duplicates)
    console.log('\n📝 Adding new permissions...');
    const insertedPermissions = [];

    for (const perm of newPermissions) {
      // Check if permission already exists
      const [existing] = await connection.execute(
        'SELECT id, module, action FROM permissions WHERE module = ? AND action = ?',
        [perm.module, perm.action]
      );

      if (existing.length > 0) {
        console.log(`⚠️  Permission ${perm.module}.${perm.action} already exists, skipping...`);
        insertedPermissions.push(existing[0]);
      } else {
        const permId = uuidv4();
        await connection.execute(`
          INSERT INTO permissions (id, module, action, name, description, is_active)
          VALUES (?, ?, ?, ?, ?, TRUE)
        `, [permId, perm.module, perm.action, perm.name, perm.description]);

        console.log(`✅ Added permission: ${perm.module}.${perm.action} - ${perm.name}`);
        insertedPermissions.push({ id: permId, module: perm.module, action: perm.action });
      }
    }

    // Update system roles to include new permissions
    console.log('\n📋 Updating system roles with new permissions...');

    // Get all permission IDs for both modules
    const [allPermissions] = await connection.execute(
      'SELECT id, module, action FROM permissions WHERE (module = ? OR module = ?) AND is_active = TRUE',
      ['departments', 'designations']
    );

    const permissionMap = {};
    allPermissions.forEach(p => {
      permissionMap[`${p.module}.${p.action}`] = p.id;
    });

    console.log('\n📊 Available permissions:', Object.keys(permissionMap));

    // Define which roles should get which permissions
    const rolePermissionUpdates = {
      'hr-admin': [
        'departments.view',
        'departments.create',
        'departments.edit',
        'departments.delete',
        'designations.view',
        'designations.create',
        'designations.edit',
        'designations.delete'
      ],
      'super-admin': [
        'departments.view',
        'departments.create',
        'departments.edit',
        'departments.delete',
        'designations.view',
        'designations.create',
        'designations.edit',
        'designations.delete'
      ]
    };

    // Add permissions to roles
    for (const [roleId, permissions] of Object.entries(rolePermissionUpdates)) {
      // Check if role exists
      const [roleCheck] = await connection.execute(
        'SELECT id, name FROM roles WHERE id = ?',
        [roleId]
      );

      if (roleCheck.length === 0) {
        console.log(`⚠️  Role ${roleId} not found, skipping...`);
        continue;
      }

      console.log(`\n🔄 Updating role: ${roleCheck[0].name} (${roleId})`);

      for (const permKey of permissions) {
        const permissionId = permissionMap[permKey];

        if (!permissionId) {
          console.log(`  ⚠️  Permission ${permKey} not found`);
          continue;
        }

        // Check if role already has this permission
        const [existingRolePerm] = await connection.execute(
          'SELECT id FROM role_permissions WHERE role_id = ? AND permission_id = ?',
          [roleId, permissionId]
        );

        if (existingRolePerm.length > 0) {
          console.log(`  ⏭️  ${permKey} already assigned to ${roleId}`);
        } else {
          await connection.execute(`
            INSERT INTO role_permissions (id, role_id, permission_id, granted_at)
            VALUES (?, ?, ?, NOW())
          `, [uuidv4(), roleId, permissionId]);

          console.log(`  ✅ Added ${permKey} to ${roleId}`);
        }
      }
    }

    // Display summary
    console.log('\n\n📊 Migration Summary:');
    console.log('='.repeat(50));

    const [departmentsPerms] = await connection.execute(`
      SELECT module, action, name, description
      FROM permissions
      WHERE module = 'departments' AND is_active = TRUE
      ORDER BY action
    `);

    console.log('\n✅ Departments Permissions:');
    departmentsPerms.forEach(p => {
      console.log(`   • ${p.module}.${p.action} - ${p.name}`);
      console.log(`     ${p.description}`);
    });

    const [designationsPerms] = await connection.execute(`
      SELECT module, action, name, description
      FROM permissions
      WHERE module = 'designations' AND is_active = TRUE
      ORDER BY action
    `);

    console.log('\n✅ Designations Permissions:');
    designationsPerms.forEach(p => {
      console.log(`   • ${p.module}.${p.action} - ${p.name}`);
      console.log(`     ${p.description}`);
    });

    console.log('\n✅ Departments and Designations permissions migration completed successfully!');
    console.log('='.repeat(50));

  } catch (error) {
    console.error('\n❌ Migration failed:', error);
    throw error;
  } finally {
    await connection.end();
  }
};

// Run migration if called directly
if (require.main === module) {
  addDepartmentsAndDesignationsPermissions()
    .then(() => {
      console.log('\n✅ Done!');
      process.exit(0);
    })
    .catch(error => {
      console.error('\n❌ Error:', error);
      process.exit(1);
    });
}

module.exports = addDepartmentsAndDesignationsPermissions;
