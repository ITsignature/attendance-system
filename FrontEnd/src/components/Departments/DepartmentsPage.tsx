...
import { Table, Button, TextInput, Modal, Label, Alert, Textarea } from "flowbite-react";
import { HiOutlinePlus, HiTrash, HiPencil, HiX } from "react-icons/hi";
...
const ManageDepartments = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await apiService.apiCall("/api/departments/with-designations");
  ...
  const handleSaveDepartment = async () => {
    ...
    try {
      if (editingDept) {
        ...
        // Create
        await apiService.apiCall("/api/departments", {
          method: "POST",
          body: JSON.stringify(deptFormData),
          headers: { "Content-Type": "application/json" },
        });
  ...
  const handleDeleteDepartment = async () => {
    ...
    try {
      await apiService.apiCall(`/api/departments/${deptToDelete.id}`, {
        method: "DELETE",
      });
  ...
  const handleSaveDesignation = async () => {
    ...
    try {
      if (editingDesig) {
        ...
        // Create
        await apiService.apiCall("/api/designations", {
          method: "POST",
          body: JSON.stringify({
            title: desigFormData.name,
  ...
  const handleDeleteDesignation = async () => {
    ...
    try {
      await apiService.apiCall(`/api/designations/${desigToDelete.id}`, {
        method: "DELETE",
      });
                  ...
                  .map((d) => (
                    <Table.Row key={d.id}>
                      <Table.Cell>
                        ...
                        {d.responsibilities && (
                          <div className="text-xs text-gray-500">{d.responsibilities}</div>
                        )}
                      </Table.Cell>
                      <Table.Cell>
                        <div className="flex gap-2">
