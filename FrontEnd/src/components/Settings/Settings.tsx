...
useEffect(() => {
  ...
  const settingSections = [
    { id: 'general', label: 'General', icon: Globe, permission: 'settings.attendance.view' },
            ...
            <div className="grid grid-cols-1 gap-6">
              ...
              <div>
                <label className="flex items-center">
                  <input
                    type="checkbox"
                    checked={localSettings.nopay_includes_allowances || false}
                    onChange={(e) => updateLocalSetting('nopay_includes_allowances', e.target.checked)}
                    className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                  />
                  <span className="ml-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                    Include Allowances in No-Pay Rate
                  </span>
                </label>
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                  When enabled, unpaid leave and absent-day deductions are calculated on (Base Salary + Allowances) / 30 instead of Base Salary alone. Late arrival / early departure deductions are not affected.
                </p>
              </div>
